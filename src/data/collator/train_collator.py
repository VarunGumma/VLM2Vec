    ...
    for k in keys:
        if isinstance(arg_val[k], torch.Tensor):
            ...
            chunked_tensor = [
                ...
                for i in list(range(0, len(arg_val[k]), chunk_size))
            ]
...
def split_vlm_inputs(model_input: dict, chunk_size: int):
    ...
    chunked_tensors = [
        arg_val[k].split(chunk_size, dim=0) for k in ["input_ids", "attention_mask"]
    ]
    ...
    positions = torch.nonzero(
        (input_ids < 0) & (input_ids > -PHI_IMAGE_TOKEN_MAX_INPUT_ID), as_tuple=True
    )
    row_contain_image = torch.unique(
        positions[0]
    )  # indicates which row in input_ids contain images
    num_chunks = len(chunked_tensors[0])
    chunk_image_count = []
    for chunk_idx in range(num_chunks):
        chunk_image_count.append(
            torch.sum(
                (row_contain_image >= chunk_idx * chunk_size)
                & (row_contain_image < (chunk_idx + 1) * chunk_size)
            ).item()
        )
    ...
    for kk, tt in zip(repeat(keys), zip(*chunked_tensors)):
        if (
            "pixel_values" in keys and tt[2].numel() == 0
        ):  # this chunk doesn't contain image
...
class MultimodalDataCollator:
                ...
                if type(raw_images) == dict:
                    ...
                    for image_idx in range(num_images):
                        ...
                        if image is not None and (
                            self.data_args.image_decay_factor is not None
                            and image_resolution is None
                        ):
                            assert (
    ...
    def __call__(self, examples):
        ...
        processed_pos_inputs = process_fn(
            pos_inputs, processor=self.processor, max_length=self.data_args.max_len
        )
        processed_qry_inputs["text"] = [e["query_text"] for e in examples]
        processed_pos_inputs["text"] = [e["pos_text"] for e in examples]
        processed_qry_inputs["global_dataset_name"] = [
            e["global_dataset_name"] for e in examples
        ]
        processed_pos_inputs["global_dataset_name"] = [
            e["global_dataset_name"] for e in examples
        ]
