...
class EvalCollator:
    ...
    def __call__(self, examples):
        ...
        """
        examples = {
            "text": [e[0] for e in examples],
            "images": [e[1] for e in examples],
        }
        inputs = process_vlm_inputs_fns[self.model_args.model_backbone](
            examples, processor=self.processor, max_length=self.data_args.max_len
        )
        inputs["texts"] = examples["text"]
        inputs["images"] = examples["images"]
        inputs["image_paths"] = [
            i.filename if hasattr(i, "filename") else None for i in examples["images"]
        ]
        return inputs
    ...
    def _get_batch_inputs(self, examples):
        ...
        for example in examples:
            ...
            text_inputs = self.txt_processors(
                text,
                padding=getattr(self.data_args, "padding", True),
        ...
        if text_exist:
            input_ids = torch.nn.utils.rnn.pad_sequence(
                input_ids,
                batch_first=True,
                padding_value=self.txt_processors.pad_token_id,
            )
...
class MultimodalEvalDataCollator:
    ...
    def _get_batch_inputs(self, batch, text_keyname, image_keyname):
        ...
        for example in batch:
            if example is None or not example:
                ...
                has_image = isinstance(ex_images, dict) or (
                    isinstance(ex_images, list)
                    and all(isinstance(item, dict) for item in ex_images)
                )
                if has_image:
                    for text, raw_images in zip(ex_text, ex_images):
                        visual_input = []
                        assert (
                            "resolutions" in raw_images
                        ), "we need len(raw_images['resolutions']) to determine the number of images, set it a list of None of for cases that no resizing is needed"
                        num_images = len(raw_images["paths"])
                        for image_idx in range(num_images):
                            bytes = (
                                raw_images["bytes"][image_idx]
                                if "bytes" in raw_images
                                else None
                            )
                            path = (
                                raw_images["paths"][image_idx]
                                if "paths" in raw_images
                                else None
                            )
                            image_resolution = (
                                raw_images["resolutions"][image_idx]
                                if "resolutions" in raw_images
                                else None
                            )
                            ...
                            if (
                                not self.data_args.resize_use_processor
                            ...
                            if image is not None and (
                                ...
                                and self.data_args.image_decay_factor is not None
                            ):
                                assert (
                                    image_resolution is None
                                ), "image_resolution is conflicting with image_decay_factor"
                                ...
                                max_pixels = max(
                                    ...
                                    self.data_args.resize_max_pixels
                                    * self.data_args.image_decay_factor
    ...
    def __call__(self, examples):
        ...
        if self.encode_side == "qry":
            assert (
                type(examples[0]["query_text"]) == list
                or type(examples[0]["query_image"]) == list
            ), "Expect text/image to be a list, even it only contains a single element (string, dict or None)"
            inputs = self._get_batch_inputs(examples, "query_text", "query_image")
        else:
            assert (
                type(examples[0]["cand_text"]) == list
                or type(examples[0]["cand_image"]) == list
            ), "Expect text/image to be a list, even it only contains a single element (string, dict or None)"
            inputs = self._get_batch_inputs(examples, "cand_text", "cand_image")

        processed_inputs = process_fn(
            inputs, processor=self.processor, max_length=self.data_args.max_len
        )
