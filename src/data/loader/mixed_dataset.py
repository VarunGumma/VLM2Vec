...
def init_mixed_dataset(dataset_config, model_args, data_args, training_args):
    ...
    train_datasets = []
    for data_idx, (global_dataset_name, dataset_config) in enumerate(
    ...
    if (
        training_args.interleave_batch_size
        and training_args.interleave_batch_size <= 1.0
    ):
    ...
    if torch.distributed.is_initialized():
        train_dataset = split_dataset_by_node(
            train_dataset, rank=torch.distributed.get_rank(), world_size=world_size
        )
