...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for query, prompt, image, image_filename, answer, answer_type, source in zip(
        batch_dict["query"],
        batch_dict["prompt"],
        batch_dict["image"],
        batch_dict["image_filename"],
        batch_dict["answer"],
        batch_dict["answer_type"],
        batch_dict["source"],
    ):
        # ignore prompt since most prompts too long
        query = process_query(
            query, prompt="", image_token=VLM_IMAGE_TOKENS[model_backbone]
        )
