...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for query, image, source in zip(
        batch_dict["query"], batch_dict["image"], batch_dict["source"]
    ):
        # ignore prompt since most prompts too long
        query = process_query(
            query, prompt=query_source2prompt.get(source, ""), image_token=""
        )
        query_texts.append(query)
        pos_text = process_query(
            "",
            prompt=target_source2prompt.get(source, ""),
...
def load_visreg_dataset(model_args, data_args, training_args, *args, **kwargs):
    ...
    global_dataset_name = kwargs.get(
        "global_dataset_name", f"{DATASET_PARSER_NAME}/{dataset_name}"
    )
