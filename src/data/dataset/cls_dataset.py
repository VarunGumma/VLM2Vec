...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for query_video_id, pos_text, video_path in zip(
        batch_dict["video_id"], batch_dict["pos_text"], batch_dict["video_path"]
    ):
        query_video_id = str(query_video_id)
        video_path = os.path.join(video_dir, video_path)
        video_path = rf"{video_path}"  # treat it as r-string to avoid path issues
        ...
        query_texts.append(
            process_input_text(
                DATASET_INSTRUCTION[dataset_name], model_backbone, add_video_token=True
            )
        )
        query_images.append(
            {
