...
class TextDataset(Dataset):
    def __init__(
        ...
        self.enable_cross_batch_negative_sampling = DATA[data_name].get(
        ...
        self.data, self.cluster = self.get_data(
            data_name, data_path, number_training_samples
        )

    def get_data(
        self, data_name: str, data_path: str = None, number_data: int = 1_000_000
    ):
        ...
        if len(dataset) > number_data:
            ...
            cluster_with_id = dataset.map(
                lambda example, idx: {"id": idx, "cluster": example["cluster"]},
                with_indices=True,
            ...
            # group by cluster
            cluster_with_id = (
                cluster_with_id.groupby("cluster")["id"].apply(list).reset_index()
            )
            ...
            for clus in cluster_with_id:
                ...
                in_cluster_index = self.rng.sample(
                    in_cluster_index, min(len(in_cluster_index), example_per_cluster)
                )
        ...
        print_master(
            f"Assigning cluster to each example for the dataset {data_name} of size {len(dataset)}..."
        )
        cluster = dataset.map(
            lambda example, idx: {"cluster": example["cluster"], "id": idx},
            with_indices=True,
            num_proc=max_num_worker_suggest,
            remove_columns=dataset.column_names,
            load_from_cache_file=True,
    ...
    def __getitem__(self, idx):
        ...
        pos = self.rng.sample(
            example["positive"], min(len(example["positive"]), self.pos_per_sample)
        )
        neg = self.rng.sample(
            example["negative"], min(len(example["negative"]), self.neg_per_sample)
        )
        assert (
            len(pos) > 0
        ), "At least one positive example per sample, got {} in idx {}. Please check the data {}".format(
            example, idx, self.data_name
        )
        assert (
            len(neg) > 0
        ), "At least one negative example per sample, got {} in idx {}. Please check the data {}".format(
            example, idx, self.data_name
        )
        return {
            "query_label": idx,
...
class RepLearningDataModule(L.LightningDataModule):
    def __init__(
        ...
        lang_to_data = {
            "en": (
                EN_CROSS_BATCH
                if use_retrieval_data_only
                else EN_CROSS_BATCH + EN_NON_CROSS_BATCH
            ),
            "ar": AR,
            "bn": BN,
            "de": DE,
    ...
    def connect(
        ...
        if self.global_batch_size % self.world_size != 0:
            ...
            print(
                f"Global batch size must be divisible by world size. Setting global batch size to {self.global_batch_size}"
            )
        if self.batch_size <= 0:
            self.batch_size = 1
            self.global_batch_size = self.world_size
            print(
                f"Batch size must be greater than 0. i.e. world_size must be less than or equal to global_batch_size. Setting batch size to {self.batch_size}"
            )
    ...
    def setup(self, stage: str = "") -> None:
        ...
        ), f"No datasets loaded. Please check the data names: {self.data_names}"
...
def get_dataloaders(
    ...
    print(
        f"Creating dataloaders for epoch {epoch} with cross_batch_loss={is_cross_batch_loss}"
    )
    batch_size = (
        training_args.global_batch_size
        if is_cross_batch_loss
        else training_args.gc_chunk_size * fabric.world_size
    )
    ...
    with fabric.rank_zero_first():
        ...
        train_dataloader = fabric.setup_dataloaders(
            train_dataloader, use_distributed_sampler=False, move_to_device=True
        )
...
if __name__ == "__main__":
    ...
    dm = RepLearningDataModule(
        langs=["en"], num_workers=0, seed=777, use_retrieval_data_only=False
    )
    dm.connect(
        world_size=4,
        global_rank=0,
