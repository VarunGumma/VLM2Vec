...
def data_prepare(
    ...
    **kwargs,
    ...
    ex_ids = []
    all_labels = (
        CLASSIFICATION_NAME2LABELS[dataset_name]
        if dataset_name in CLASSIFICATION_NAME2LABELS
        else []
    )
    ...
    if data_type == "against_text":
        for data_idx, text in enumerate(examples["text"]):
            if local_load:
                ...
                pos = (
                    example["pos"].lower().strip()
                    if "pos" in example
                    else example["label_text"]
                )
            else:
            ...
            if num_hardneg > 0:
                if len(negs) > 0:
                    ...
                    negs = np.random.choice(
                        negs, size=num_hardneg, replace=len(negs) < num_hardneg
                    )
                    ...
                    if data_type == "against_text":
                        ...
                        negs = np.random.choice(
                            _neg_texts,
                            size=num_hardneg,
                            replace=len(_neg_texts) < num_hardneg,
                        )
    ...
    **kwargs,
    ...
    if os.path.isfile(file_path):
        ...
        if (
            ...
            and num_sample_per_subset < dataset.num_rows
        ):
        ...
        # load from HF hub
        config_name = (
            "en" if "en" in datasets.get_dataset_config_names(file_path) else None
        )
    ...
    dataset = dataset.map(
        ...
        batched=True,
        batch_size=512
        * 4,  # batch size has to be smaller than each data size due to bugs in datasets>=2.19.2
