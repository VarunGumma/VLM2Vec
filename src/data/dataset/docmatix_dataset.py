...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for chats, images in zip(batch_dict["texts"], batch_dict["images"]):
        ...
        pos_texts.append(
            f"{DOCMATIX_DOC_PROMPT} {''.join([VLM_IMAGE_TOKENS[model_backbone]] * len(images))}"
        )
...
def load_docmatix_dataset(model_args, data_args, training_args, *args, **kwargs):
    ...
    assert (
        "config_name" in kwargs
    ), "config_name must be provided for `docmatix`, ['images', 'pdf', 'zero-shot-exp']"
