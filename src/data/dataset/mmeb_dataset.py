...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for (
        ...
        batch_dict["qry"],
        batch_dict["qry_image_path"],
        batch_dict["pos_text"],
        batch_dict["pos_image_path"],
        batch_dict.get("neg_text", [""] * batch_size),
        batch_dict.get("neg_image_path", [None] * batch_size),
    ):
        if (not qry_text and not qry_image_path) or (
            not pos_text and not pos_image_path
        ):
            print("empty inputs")
            continue
        ...
        qry_image = {
            "bytes": [None],
            "paths": [
                os.path.join(image_dir, qry_image_path) if qry_image_path else None
            ],
        ...
        pos_image = {
            "bytes": [None],
            "paths": [
                os.path.join(image_dir, pos_image_path) if pos_image_path else None
            ],
            "resolutions": [RESOLUTION_MAPPING.get(image_resolution, None)],
        }
        neg_image = {
            "bytes": [None],
            "paths": [
                os.path.join(image_dir, neg_image_path) if neg_image_path else None
            ],
            "resolutions": [RESOLUTION_MAPPING.get(image_resolution, None)],
        }
