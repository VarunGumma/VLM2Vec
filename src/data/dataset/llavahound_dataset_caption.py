...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for data_idx, (data_id, conversations, video_id) in enumerate(
        ...
        try:
            if data_mode == "caption_retrieval":
                query, pos_text = process_conversations(
                    conversations, video_token=VLM_VIDEO_TOKENS[model_backbone]
                )
                ...
                assert (
                    len(frame_paths) > 0
                ), f"llavahound_dataset_caption.py: No frames found for video_id={video_id} at {os.path.join(frame_basedir, video_id)}."
                video_frames = {
                ...
                query = process_conversations_for_vret(
                    conversations, prompt=VRET_QRY_PROMPT
                )
                frame_paths = process_video_frames(
                    os.path.join(frame_basedir, video_id), num_frames=num_frames
                )
            ...
            print(
                f"Error in processing {DATASET_PARSER_NAME}: \n\t\tdata id: {data_id} \n\t\tconversations: {conversations}"
            )
            print(e)
            raise e
...
def load_llavahound_caption_dataset(
    model_args, data_args, training_args, *args, **kwargs
):
    dataset_name = kwargs.get("dataset_name", DATASET_PARSER_NAME)
    assert (
        "dataset_path" in kwargs
    ), "`dataset_path` should be given for loading llavahound dataset."
    assert (
        "num_frames" in kwargs
    ), "`num_frames` should be given for loading llavahound dataset."
    assert (
        "video_frame_basedir" in kwargs
    ), "`video_frame_basedir` should be given for loading llavahound dataset."
    dataset_path = kwargs["dataset_path"]
    dataset = datasets.load_dataset(
        "json", split="train", data_files=dataset_path, streaming=False
    )
