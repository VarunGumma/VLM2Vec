...
def ex_dict2str(ex_dict, add_title=True):
    ...
    if not isinstance(ex_dict, dict):
        raise NotImplementedError(
            f"Wrong ex_dict object type, expecting dict: {ex_dict}"
        )
...
def data_prepare(
    ...
    dataset_name=None,
    num_hardneg=0,
    query_group_name=None,
    query_group_range=None,
    hardneg_range=None,
    neg_score_threshold=None,
    posneg_margin=None,
    posneg_ratio=None,
    posbased_filter_range=None,
    posbased_filter_score=None,
    prevneg_ratio=0.0,
    query_prompt="",
    doc_prompt="",
    *args,
    **kwargs,
    ...
    for data_idx, text in enumerate(batch_dict["text"]):
        try:
            ...
            assert isinstance(example["pos"], list) and isinstance(
                example["neg"], list
            ), f'Expect pos/neg is a list of strings or dicts, but got {type(example["pos"])} and {type(example["neg"])}'
            ...
            if num_hardneg > 0:
                ...
                if (
                    "dataset" in example
                    and "arguana" in example["dataset"]
                    and "neg_docids" in example
                ):
                    # specific to arguana, treating passages from the same doc as positives
                    pos_docid_prefix = example["pos"][0]["doc_id"][
                        : example["pos"][0]["doc_id"].rindex("-")
                    ]
                    for docid in example["neg_docids"]:
                        if docid.startswith(pos_docid_prefix):
                            docids_to_filter.append(docid)
                if (
                    posbased_filter_range or posbased_filter_score
                ) and "posbased_neg_docids" in example:
                    if posbased_filter_range:
                        ...
                        r_end = (
                            int(posbased_range[1])
                            if posbased_range[1]
                            else len(example["posbased_neg"])
                        )
                        docids_to_filter.extend(
                            example["posbased_neg_docids"][r_start:r_end]
                        )
                    if posbased_filter_score:
                        for score, docid in zip(
                            example["posbased_neg_scores"],
                            example["posbased_neg_docids"],
                        ):
                ...
                if len(docids_to_filter) > 0:
                    valid_neg_idxs = [
                        neg_idx
                        for neg_idx, neg_docid in enumerate(example["neg_docids"])
                        if neg_docid not in docids_to_filter
                    ]
                ...
                if neg_score_threshold is not None:
                    neg_score_tuples = [
                        t for t in neg_score_tuples if t[1] < neg_score_threshold
                    ]
                ...
                if posneg_margin is not None and "pos_score" in example:
                    ...
                    neg_score_tuples = [
                        t for t in neg_score_tuples if t[1] < pos_score - posneg_margin
                    ]
                ...
                if posneg_ratio is not None and "pos_score" in example:
                    pos_score = example["pos_score"]
                    neg_score_tuples = [
                        t for t in neg_score_tuples if t[1] < pos_score * posneg_ratio
                    ]
                ...
                if (
                    prevneg_ratio > 0
                    and random.random() < prevneg_ratio
                    and "prev_neg" in example
                    and len(example["prev_neg"])
                ):
                    prev_negs = np.random.choice(
                        example["prev_neg"],
                        size=min(
                            len(example["prev_neg"]),
                            math.ceil(num_hardneg * prevneg_ratio),
                        ),
    ...
    return_dict = {
        "query_text": queries,
        "query_image": [None] * batch_len,
        "pos_text": pos_docs,
        "pos_image": [None] * batch_len,
        "neg_text": neg_docs if neg_docs else [None] * batch_len,
        "neg_image": [None] * batch_len,
    }
    return return_dict
...
def load_beir_dataset(
    ...
    dataset_name,
    file_path,
    query_prompt_type="e5mistral",
    doc_prompt_type="e5mistral",
    *args,
    **kwargs,
):
    # set up prompt
    ...
    if query_prompt_type:
        query_prompt = AutoPrompt.instantiate(
            prompt_family=query_prompt_type, task_name=prompt_name, **kwargs
        )["q_prompt"]
    if doc_prompt_type:
        doc_prompt = AutoPrompt.instantiate(
            prompt_family=doc_prompt_type, task_name=prompt_name, **kwargs
        )["d_prompt"]
    ...
    dataset = datasets.load_dataset(
        "text", data_files=file_path, split="train", keep_in_memory=False
    )
    ...
    )  # convert to IterableDataset and multiple shards
    num_hardneg = kwargs.get("num_hardneg", data_args.num_hardneg)
    dataset = dataset.map(
        lambda x: data_prepare(
            x,
