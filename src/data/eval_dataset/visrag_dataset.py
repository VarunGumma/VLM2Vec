...
from src.data.utils.dataset_utils import (
    load_hf_dataset,
    sample_dataset,
    load_qrels_mapping,
)
    ...
    for query_id, query in zip(batch_dict["query-id"], batch_dict["query"]):
        ...
        for image_name, rel_score in qrels_mapping[query_id].items():
            ...
            short_base = (
                base[:50]
                + "_"
                + hashlib.md5(image_name.encode("utf-8")).hexdigest()[:8]
            ...
            cand_text.append(
                process_input_text(TASK_INST_TGT, model_backbone, add_image_token=True)
            )
            cand_image.append(
                ImageVideoInstance(
                    bytes=[None],
...
def corpus_prepare(batch_dict, *args, **kwargs):
    ...
    for image_name, image in zip(batch_dict["corpus-id"], batch_dict["image"]):
        ...
        base, ext = os.path.splitext(image_name)
        short_base = (
            base[:50] + "_" + hashlib.md5(image_name.encode("utf-8")).hexdigest()[:8]
        )  # Truncate base, add original filename hash
        new_imagename = short_base + ext
        image_path = f"{image_root}/{new_imagename}"
        if not os.path.exists(image_path):
