...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    if "qry_text" in batch_dict:
        ...
        assert (
            "cand_text" in batch_dict
        ), "either 'qry_text' or 'cand_text' must be in the batch_dict"
    ...
    for qry_text, qry_image_path, cand_id, retrieval_type in zip(
        batch_dict[text_keyname],
        batch_dict[image_keyname],
        batch_dict["cand_id"],
        batch_dict.get("retrieval_type", [None] * batch_size),
    ):
    ...
    if text_keyname == "qry_text":
        ...
        return {
            "cand_text": query_texts,
            "cand_image": query_images,
            "dataset_infos": dataset_infos,
        }
