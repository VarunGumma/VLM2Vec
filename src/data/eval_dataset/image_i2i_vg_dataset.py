...
def data_prepare(batch_dict, *args, **kwargs):
    image_resolution, model_backbone = (
        kwargs["image_resolution"],
        kwargs["model_backbone"],
    )
    ...
    for qry_inst, qry_text, qry_img_path, tgt_inst, tgt_captions, tgt_img_paths in zip(
        ...
        qry_text = process_input_text(
            qry_inst, model_backbone, text=qry_text, add_image_token=True
        )
        ...
        if tgt_captions[0].strip():  # RefCOCO-Matching has valid text inputs
            ...
            tgt_inst = tgt_inst.replace("<|image_1|>", "")
            tgt_inst_caption = process_input_text(
                tgt_inst, model_backbone, text="", add_image_token=True
            )
            tgt_inst_caption = tgt_inst_caption.replace(
                " \n", "\n"
            )  # to stay consistent with v1 eval
        ...
        cand_img_paths = [
            os.path.join(image_root, tgt_img_path) for tgt_img_path in tgt_img_paths
        ]
        img_list = [
            {
                "bytes": [None],
        ...
        cand_names = [
            path + ":" + cap.strip('"')
            for path, cap in zip(tgt_img_paths, tgt_captions)
        ]
