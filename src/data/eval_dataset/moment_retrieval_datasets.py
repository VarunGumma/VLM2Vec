...
from src.utils.vision_utils.vision_utils import (
    save_frames,
...
def data_prepare(batch_dict, *args, **kwargs):
    ...
    for query, query_video_path in zip(batch_dict["query"], batch_dict["video_path"]):
        ...
        # Load query video
        query_video_path = (
            os.path.join(video_root, os.path.basename(query_video_path))
            if video_root
            else None
        )
        ...
        qry_frame_paths = process_video_frames(
            query_frame_dir, num_frames=num_video_frames
        )

        query_texts.append(
            [
                process_input_text(
                    TASK_INST_QRY, model_backbone, text=query, add_video_token=True
                )
            ]
        )
        query_images.append(
        ...
        if not os.path.exists(frames_dir):
            ...
            clip_video_paths = [
                f
                for f in os.listdir(clip_video_dir)
        ...
        for clip_frame_dir_or_file in os.listdir(frames_dir):
            ...
            cand_frame_paths = process_video_frames(
                clip_frame_dir_abs, num_frames=num_clip_frames
            )
            cand_frames.append(
                ImageVideoInstance(
                    ...
                    resolutions=[RESOLUTION_MAPPING.get(image_resolution, None)]
                    * len(cand_frame_paths),
                ).to_dict()
            )
            cand_clip_names.append(
                clip_frame_dir_abs
            )  # use absolute path here instead of file name to keep it unique
        cand_texts.append(
            [process_input_text(TASK_INST_TGT, model_backbone, add_video_token=True)]
            * len(cand_clip_names)
        )
