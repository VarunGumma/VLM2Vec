...
EVAL_QRY_FEATURES = Features(
    **{
        "query_text": Sequence(
            Value(dtype="string")
        ),  # Only one element, but make it as a sequence for collator usage
        "query_image": Sequence(
            {
                "paths": Sequence(
                    Value(dtype="string")
                ),  # List of image paths (frames)
                "bytes": Sequence(
                    Value(dtype="binary")
                ),  # List of pre-saved image bytes
        ...
        "cand_text": Sequence(
            Value(dtype="string")
        ),  # Here it's only for hard negatives.
        "cand_video": Sequence(
            {
...
def generate_cand_dataset(dataset, corpus):
    ...
    for row in dataset:
        assert (
            len(row["cand_text"])
            == len(row["cand_image"])
            == len(row["dataset_infos"]["cand_names"])
        )
        for cand_text, cand_image, cand_name in zip(
            row["cand_text"], row["cand_image"], row["dataset_infos"]["cand_names"]
        ):
