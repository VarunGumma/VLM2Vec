...
def load_e5mistral_multilingual_prompt(task_name, task_type, *args, **kwargs):
    if not task_type:
        task_type = "Retrieval"
    if (
        task_name.endswith("_small")
        or task_name.endswith("_s")
        or task_name.endswith("_xs")
    ):
        task_name = task_name[: task_name.rindex("_")]
    if task_name.startswith("cqadupstack-"):
        task_name = "cqadupstack"
    task_def = get_task_def_by_task_name_and_type(
...
def get_task_def_by_task_name_and_type(task_type: str, task_name: str) -> str:
    ...
    raise ValueError(
        f"No instruction config for task_name `{task_name}` with task_type `{task_type}`"
    )
