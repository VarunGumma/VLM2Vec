...
def is_binary_tensor(tensor):
    ...
    return torch.equal(
        unique_elements,
        torch.tensor([0, 1], dtype=tensor.dtype).to(unique_elements.device),
    )
...
class BiEncoderGradCache(nn.Module):
    ...
    def build_cache(
        self, deepspeed=None, loss_mapping=None, *reps: Tensor, **loss_kwargs
    ) -> [List[Tensor], Tensor]:
        """
    ...
    def cache_step(
        ...
        **loss_kwargs,
        ...
        inputs = [
            ...
            for x, chunk_size in zip(inputs, self.chunk_sizes)
        ]
        masks = [
            self.split_inputs(x, chunk_size)
            for x, chunk_size in zip(masks, self.chunk_sizes)
        ]
        ...
        for model, input, mask in zip(self.models, inputs, masks):
            if len(input[0].shape) == 1 or is_binary_tensor(input[0]):
                ...
                model_reps, rnd_states = self.forward_no_grad(
                    model, model_inputs=(input, mask)
                )
                all_reps.append(model_reps)
                all_rnd_states.append(rnd_states)

        # print('start to build cache')
        cache, loss, loss_details = self.build_cache(
            deepspeed, loss_mapping, *all_reps, **loss_kwargs
        )
        cache = [c.split(chunk_size) for c, chunk_size in zip(cache, self.chunk_sizes)]

        for model, input, mask, model_cache, rnd_states in zip(
            self.models, inputs, masks, cache, all_rnd_states
        ):
            self.forward_backward(
                model,
                model_inputs=list(zip(input, mask)),
                cached_gradients=model_cache,
        ...
        log_stats = BiEncoder._report_train_metrics(
            q=all_reps[0],
            k=all_reps[1],
            preds=loss_details["preds"],
            labels=loss_details["labels"],
