...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        try:
            ...
            key_states = key_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
            value_states = value_states.view(bsz, q_len, -1, self.head_dim).transpose(
                1, 2
            )
        except RuntimeError:
        ...
        if inputs_embeds is None:
            ...
            if len(idx_w_image) > 0:
                ...
                if len(idx_w_image):
                    ...
                    valid_image_sizes = torch.cat(valid_image_sizes).to(
                        input_ids.device
                    )  # shape=[BS,H,W]

                    valid_pixel_values = valid_pixel_values.type(self.visual.dtype)
                    image_embeds = self.visual(
                        valid_pixel_values, grid_thw=valid_image_sizes
                    )
                    n_image_tokens = (
                        (input_ids_w_image == self.config.image_token_id).sum().item()
                    )
