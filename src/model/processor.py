...
MODEL2BACKBONE = (
    {  # keys are from hf_config.model_type or manually added if not provided
        "phi3_v": PHI3V,
        "llava_next": LLAVA_NEXT,
        "qwen2_vl": QWEN2_VL,
        "qwen2_vl_tokenselection": QWEN2_VL,
...
def load_processor(model_args, data_args=None):
    ...
    if model_args.model_backbone == PHI3V:
        ...
        processor = Phi3VProcessor.from_pretrained(
            model_name_or_path, trust_remote_code=True, num_crops=model_args.num_crops
        )
        ...
        from src.model.vlm_backbone.qwen2_vl.image_processing_qwen2_vl import (
        ...
        from src.model.vlm_backbone.qwen2_vl_tokenselection.processing_qwen2_vl import (
            Qwen2VLProcessor,
        )
        from src.model.vlm_backbone.qwen2_vl_tokenselection.image_processing_qwen2_vl import (
            Qwen2VLImageProcessor,
        )
        from src.model.vlm_backbone.qwen2_vl_tokenselection.tokenization_qwen2_fast import (
            Qwen2TokenizerFast,
        )
        ...
        from src.model.vlm_backbone.qwen2_5_vl_tokenselection.processing_qwen2_5_vl import (
            Qwen2_5_VLProcessor,
        )
...
def get_backbone_name(hf_config, model_type=None):
    ...
    assert (
        hf_config.model_type in SUPPORTED_MODELS
    ), f"Unknown backbone name {hf_config.model_type}.Supported models are {SUPPORTED_MODELS}"
...
def Llava_NEXT_process_fn(model_inputs: dict, processor, max_length=None):
    ...
    for text, images in zip(texts, visual_inputs):
        ...
        if images is None or (type(images) == list and any(i is None for i in images)):
            ...
            assert isinstance(
                images, list
            ), f"images should be a list, but got {type(images)}"
            inputs = processor(
                images=images,
                text=text,
                return_tensors="np",
    ...
    if image_exists:
        ...
        pixel_values = pixel_values.reshape(
            pixel_values_shape[0] * pixel_values_shape[1], *pixel_values_shape[2:]
        )
...
def Phi3V_process_fn(model_inputs: dict, processor, max_length=None):
    ...
    for text, image in zip(texts, images):
        if image is None:
            inputs = processor(
                text, None, return_tensors="np", max_length=max_length, truncation=True
            )
            input_id = inputs["input_ids"].squeeze().tolist()
            if isinstance(input_id, int):
                # in case of empty string, only BOS is included
...
def Qwen2_VL_TokenSelection_process_fn(
    ...
    for text, images in zip(texts, visual_inputs):
        if images is None or (type(images) == list and any(i is None for i in images)):
            ...
            if VLM_IMAGE_TOKENS[QWEN2_VL] in text:
                ...
                assert (
                    len(images) > 1
                ), f"Video data must have more than 1 frame, got {len(images)}"
                inputs = processor(
                    text=[text],
                    videos=[images],
                    return_tensors="np",
    ...
    if image_exists:
        if patch_pos:
            patch_pos_shape_for_padding = list(
                v.shape for v in patch_pos if v is not None
            )[0]
            key_tmp = [
                (
                    torch.from_numpy(v)
                    if v is not None
                    else (torch.zeros(patch_pos_shape_for_padding) - 1)
                )
                for v in patch_pos
            ]
        ...
        if select_mask:
            select_mask_shape_for_padding = list(
                v.shape for v in select_mask if v is not None
            )[0]
            key_tmp = [
                (
                    torch.from_numpy(v)
                    if v is not None
                    else torch.ones(select_mask_shape_for_padding).bool()
                )
                for v in select_mask
            ]
            max_length = input_ids.size(1)
            padded_key = [
    ...
    if all(x is None for x in model_inputs["images"]):
        ...
        # Case 2: frame_list is already a list of lists, ensure each has exactly 4 images
            ...
            for frames in frame_list:
                if len(frames) < 4:
                    frames = frames + [
                        frames[-1].copy() for _ in range(4 - len(frames))
                    ]
                elif len(frames) > 4:
        ...
        pixel_values = [
            torch.stack(
                [preprocess(img) for img in frames], dim=0
            )  # (num_frames, C, H, W)
            for frames in frame_list
        ]
...
def process_input_text(
    instruction, model_backbone, text=None, add_video_token=False, add_image_token=False
):
    ...
    if model_backbone == "internvideo2":
        ...
        return PROMPT_TEMPLATE_DICT[model_backbone](
            text, add_video_token, add_image_token
        )
