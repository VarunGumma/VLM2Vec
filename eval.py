...
def encode_embeddings(
    ...
    if is_late_interaction:
        if dist.is_initialized():
            ...
            local_max_len_tensor = torch.tensor(
                local_max_len, device=training_args.device
            )
    ...
    if dist.is_initialized() and full_dataset.num_rows >= world_size:
        ...
        gathered_embeds_tensor = torch.empty(
            output_shape, dtype=embeds_tensor.dtype, device=training_args.device
        )
...
def main():
    ...
    for dataset_name, task_config in dataset_configs.items():
            ...
            try:
                ...
                if dist.is_initialized():
                    ...
                    eval_qry_dataset = split_dataset_by_node(
                        padded_qry_dataset, rank=local_rank, world_size=world_size
                    )
                    eval_cand_dataset = split_dataset_by_node(
                        padded_cand_dataset, rank=local_rank, world_size=world_size
                    )
                else:
        ...
        if do_query:
            ...
            eval_qry_collator = MultimodalEvalDataCollator(
        ...
        if do_cand:
            ...
            eval_cand_collator = MultimodalEvalDataCollator(
        ...
        if local_rank == 0:
            ...
            if rank_against_all_candidates:
                ...
                if (
                    qry_embeds.ndim == 3
                ):  # Query: [N_q, L_q, H] | Candidate: [N_c, L_c, H]
                    qry_embed = torch.from_numpy(qry_embeds)
                    cand_embeds = [torch.from_numpy(np.array(t)) for t in cand_embeds]
                    scores = processor.score(
                        qry_embed, cand_embeds, batch_size=64
                    )  # use ColPali score function
                    ranked_candids = (
                        torch.argsort(-scores, dim=1).cpu().numpy().tolist()
                    )
                else:  # Dense
                    cosine_scores = np.dot(qry_embeds, cand_embeds.T)
                    ranked_candids = np.argsort(-cosine_scores, axis=1)
                for qid, (ranked_candid, gt_info) in tqdm(
                    enumerate(zip(ranked_candids, gt_infos)),
                    ...
                    rel_scores = (
                        gt_info["rel_scores"] if "rel_scores" in gt_info else None
                    )
                ...
                for qid, (qry_embed, gt_info) in tqdm(
                    enumerate(zip(qry_embeds, gt_infos)),
                    desc=f"Calculating scores for {dataset_name}",
                ):
                    cand_embeds = np.stack(
                        [cand_embed_dict[key] for key in gt_info["cand_names"]]
                    )
                    if (
                        qry_embeds.ndim == 3
                    ):  # Query: [N_q, L_q, H] | Candidate: [N_c, L_c, H]
                        ...
                        cand_embeds = [
                            torch.from_numpy(np.array(t)) for t in cand_embeds
                        ]
                        scores = processor.score(
                            qry_embed, cand_embeds, batch_size=1024
                        )  # use ColPali score function
                        ranked_candids = (
                            torch.argsort(-scores, dim=1).cpu().numpy().tolist()[0]
                        )
                    else:
                    ...
                    pred_dicts.append(
                        {
                            "prediction": [
                                gt_info["cand_names"][i] for i in ranked_candids
                            ],
                            "label": rel_docids,
                            "rel_scores": rel_scores,
                        }
            ...
            metrics_to_report = (
                task_config["metrics"]
                if task_config.get("metrics", None) is not None
                else ["hit", "ndcg", "precision", "recall", "f1", "map", "mrr"]
            )
