...
class DenseEncoder(torch.nn.Module):
    ...
    def encode(self, inputs, prompt=None, is_query=True, **kwargs) -> np.ndarray:
        ...
        if (
            ...
            and len(input_texts) >= dist.get_world_size()
        ):
            idx = np.array_split(range(len(input_texts)), dist.get_world_size())[
                dist.get_rank()
            ]
        else:
        ...
        if prompt:
            device_sentences_with_prompt = [
                prompt + (s["text"] if isinstance(s, dict) else s)
                for s in device_sentences
            ]
        else:
            device_sentences_with_prompt = device_sentences

        dataset: Dataset = Dataset.from_dict(
            {"input_texts": device_sentences_with_prompt}
        )
        ...
        data_loader = DataLoader(
            ...
            batch_size=(
                ...
                else self.batch_size_per_device * self.gpu_count
            ),
            shuffle=False,
            drop_last=False,
            num_workers=0,
        ...
        for batch in tqdm.tqdm(
            data_loader, desc="encoding", miniters=10, disable=not is_main()
        ):
...
def main():
    parser = HfArgumentParser(
        (ModelArguments, DataArguments, MTEBArguments, TrainingArguments)
    )
    model_args, data_args, mteb_args, training_args, remaining_args = (
    ...
    for task_cls in evaluation.tasks:
        ...
        if "test" not in eval_splits:
            logger.warning(
                "Test split not found for task: {}, type: {}, eval_splits: {}".format(
                    task_name, task_type, eval_splits
                )
            )
        eval_splits = ["test" if "test" in eval_splits else eval_splits[0]]

        if mteb_args.prompt_family:
            prompt_data = load_e5mistral_prompt(
                prompt_family=mteb_args.prompt_family,
                task_name=task_name,
                task_type=task_type,
            )
        ...
        sub_eval = MTEB(
            tasks=[task_name], task_langs=["eng-Latn", "en"], n_experiments=1
        )
        logger.info(
        ...
        sub_eval.run(model, eval_splits=eval_splits, output_folder=mteb_result_folder)
