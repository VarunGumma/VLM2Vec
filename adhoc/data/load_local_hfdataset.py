...
if __name__ == "__main__":
    ...
    for dataset_name, (local_path, subset, split) in EVAL_DATASET_HF_PATH.items():
        ...
        try:
            loaded_dataset = load_local_hf_dataset(
                dataset_path=local_path, subset=subset, split=split
            )
            print(
                f"  Successfully loaded {dataset_name}, Total samples: {len(loaded_dataset)}"
            )
